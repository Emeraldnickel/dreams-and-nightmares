...
public class NightmareDimensionTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("nightmare_dimension_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("nightmare_dimension_portal",
				com.google.common.collect.Sets.newHashSet(
						ImmutableSet.copyOf(DreamsAndNightmaresModBlocks.NIGHTMARE_DIMENSION_PORTAL.get().getStateDefinition().getPossibleStates())),
				0, 1).setRegistryName("nightmare_dimension_portal");
	...
	public NightmareDimensionTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
				...
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0
								? DreamsAndNightmaresModBlocks.DREAM_DOORWAY_FRAME.get().defaultBlockState()
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, DreamsAndNightmaresModBlocks.DREAM_DOORWAY_FRAME.get().defaultBlockState(), 3);
				}
			}
		}
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = NightmareDimensionPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return NightmareDimensionPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d,
